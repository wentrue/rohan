...
"""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""
   ...
   """"""""""""""""""""""""""""""
   "Run the current buffer in python - ie. on leader+space
   autocmd FileType R map <buffer> <leader><space> :w!<cr>:!Rscript %<cr>

   "Run in the Rscript interpreter
   function! R_Eval_VSplit() range
     let src = tempname()
     let dst = tempname()
     execute ": " . a:firstline . "," . a:lastline . "w " . src
     execute ":!Rscript " . src . " > " . dst
     execute ":pedit! " . dst
   endfunction
   au FileType R vmap <F7> :call R_Eval_VSplit()<cr> 
